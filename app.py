"""
FFmpeg Video Composition API - v7
Simple approach matching reference exactly:
1. Black canvas 1080x1920
2. Image fit-to-width (no crop) at Y=230
3. Avatar overlaid at 0,0 FULL SIZE (1080x1920) - no scale, no crop
   Black background from HeyGen blends with black canvas.
"""
...
# Image placement — fit to width, placed behind avatar
IMAGE_Y = 230       # top margin (black space above image)
IMAGE_HEIGHT = 480   # image display height (fit, no crop)

# Avatar: NO modification. Overlay at 0,0 full 1080x1920.
# HeyGen outputs 1080x1920 with black bg + person in lower half.
# Black on black = invisible. Person head overlaps image naturally.
...
def build_ffmpeg_command(avatar_path, image_paths, duration, output_path):
    ...
    # STEP 2: Images — FIT to width (preserve edges, pad with black)
    ...
    # STEP 3: Avatar — overlay at 0,0, FULL SIZE, no modification
    filter_parts.append(
        f"[{canvas}][{avatar_input}:v]overlay=0:0:shortest=1[out]"
    )
...
def health():
    try:
        ...
        return jsonify({'status': 'healthy', 'ffmpeg': version, 'api_version': 'v7'})
    except Exception as e:
...
def compose():
    try:
        ...
        if not image_paths:
            logger.warning(f"[{job_id}] No images downloaded")
